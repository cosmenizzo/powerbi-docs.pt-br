...
5. Para o SQL Server, você escolhe um **Método de Autenticação** do **Windows** ou **Básico** (Autenticação SQL). Se você escolher **Básico**, insira as credenciais para a fonte de dados.
    ...
    > [!NOTE]
    > Se o método de autenticação selecionado for OAuth, qualquer consulta que seja executada por mais tempo do que a política de expiração do token OAuth poderá falhar.
